...
regression_mortality <- function(outcome_var, df) {
  ...
  m1 <- coxph(as.formula(paste0("Surv(censoring_time_5y, ", outcome_var, ") ~ cluster_MOD + cluster_SIDD + cluster_SIRD + dm_age")),
              data = df 
              # weights = pooled_weight
              )
  
  m2 <- coxph(as.formula(paste0("Surv(censoring_time_5y, ", outcome_var, ") ~ cluster_MOD + cluster_SIDD + cluster_SIRD + gender + dm_age + smoke_current")),
              data = df
              # weights = pooled_weight
                        )
  
  s2 <- svycoxph(as.formula(paste0("Surv(censoring_time_5y, ", outcome_var, ") ~ cluster_MOD + cluster_SIDD + cluster_SIRD + gender + dm_age + smoke_current")),
              design = des
  )
  
  s0 <- svycoxph(as.formula(paste0("Surv(censoring_time_5y, ", outcome_var, ") ~ cluster_MOD + cluster_SIDD + cluster_SIRD")),
              design = des
  )
  
  m0 <- coxph(as.formula(paste0("Surv(censoring_time_5y, ", outcome_var, ") ~ cluster_MOD + cluster_SIDD + cluster_SIRD")),
              data = df 
              # weights = pooled_weight
              )
  ...
  mtext(paste0(disease_labels[outcome_var]," - 5 year (Unweighted)"), side = 3, line = - 2, outer = TRUE)
  
  par(mfrow=c(2,3))
  plot(NPH_CHECK)
  mtext(paste0(disease_labels[outcome_var]," - 5 year (Weighted)"), side = 3, line = - 2, outer = TRUE)
